...
/*   Updated: 2020/01/23 14:06:20 by prmerku          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_nw(char **data, t_win *win, int *i)
{
	...
	win->tex.n_wall = mlx_png_file_to_image(win->mlx, path, &width, &height);
	if (!win->tex.n_wall)
...
void	parse_sw(char **data, t_win *win, int *i)
{
	...
	if (!path)
		close_error("Malloc error\n");
	win->tex.s_wall = mlx_png_file_to_image(win->mlx, path, &width, &height);
	if (!win->tex.s_wall)
		close_error("Invalid texture\n");
...
void	parse_ew(char **data, t_win *win, int *i)
{
	...
	if (!path)
		close_error("Malloc error\n");
	win->tex.e_wall = mlx_png_file_to_image(win->mlx, path, &width, &height);
	if (!win->tex.e_wall)
		close_error("Invalid texture\n");
...
void	parse_ww(char **data, t_win *win, int *i)
{
	...
	if (!path)
		close_error("Malloc error\n");
	win->tex.w_wall = mlx_png_file_to_image(win->mlx, path, &width, &height);
	if (!win->tex.w_wall)
		close_error("Invalid texture\n");
