...
void *mlx_int_parse_xpm(t_xvar *xvar, void *info, int info_size, char *(*f)()) {
	...
	if (cpp <= 2) {
		method = 1;
		if (!(colors_direct = malloc((cpp == 2 ? 65536 : 256) * sizeof(int)))) RETURN;
	} else if (!(colors = malloc(nc * sizeof(*colors)))) RETURN;
	...
	while (i--) {
		if (!(line = f(info, &pos, info_size)) ||
			!(tab = mlx_int_str_to_wordtab(line + cpp))) RETURN;
		j = 0;
		...
		if ((rgb_col = mlx_int_get_text_rgb(tab[j], tab[j + 1])) == -1) {
			if (!(clip_data = malloc(4 * width * height)) ||   /* ok, nice size .. */
				!(clip_img = XCreateImage(xvar->display, xvar->visual,
										  ...
										  width, height, 8, (width + 7) / 8))) RETURN;
	...
	if (!(img = mlx_new_image(xvar, width, height))) RETURN;
	opp = img->bpp / 8;
	...
	data = img->data;
	while (i--) {
		if (!(line = f(info, &pos, info_size))) RETURN;
		x = 0;
		while (x < width) {
			col = 0;
